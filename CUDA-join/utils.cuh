...
__device__ __host__ void for_(F func, std::index_sequence<Is...>)
{
    (func(num<Is>{}), ...);
}
template <std::size_t N, typename F>
__device__ __host__ void for_(F func)
{
...
inline void alloc_by_cuda(void **ptr, bool clear, size_t sz, cudaStream_t stream)
{
    ...
    if (clear)
        CHECK_CUDA_ERROR(cudaMemsetAsync(*ptr, 0, sz, stream));
}
...
constexpr size_t total_gpu_memory = 11264ULL * 1024 * 1024; // 11264 MiB 转换为字节
constexpr double usage_fraction = 0.9;                      // 使用 85% 的总内存
...
{
    if (mm == nullptr)
        mm = new UserSpaceMM<mem_pool_size>();
...
__host__ __device__ inline int num_tb(const int N, const int threads_per_block = 1024, const int items_per_thread = 1)
{
