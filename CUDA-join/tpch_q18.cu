...
void execute_q18_part_tpch_all_long(int vec_size, std::string file, std::string algo)
{
    ...
    while (!t.is_end())
    {
        std::cout << "(" << t.num_items << " rows)" << std::endl;
        total_rows += t.num_items;
        batch_count++;
        std::cout << "Batch " << batch_count << ": " << t.num_items << " rows" << std::endl;
        t.free_mem();
        t = op_root.next();
    }
    op_root.close();
    ...
    printf("Total scanned rows: %d \n", total_rows);
    printf("Number of batches: %d \n", batch_count);
    printf("Total time: %.6f seconds\n", elapsed.count());
    printf("Average rows per batch: %.2f\n", batch_count > 0 ? static_cast<double>(total_rows) / batch_count : 0);

    // 输出每个批次的详细信息
    std::cout << "Detailed batch information:\n";
    for (int i = 0; i < batch_count; i++)
    {
        std::cout << "Batch " << i + 1 << ": " << t.num_items << " rows" << std::endl;
    }
}
