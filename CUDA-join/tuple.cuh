...
struct Chunk
{
    ...
    __host__ void free_mem(bool free_select_vec = true, std::vector<int> *skip = nullptr)
    {
        if (empty())
            return;
        if (free_select_vec)
        {
            if (select_vec)
                release_mem(select_vec, st_);
            select_vec = nullptr;
        }
        for (int i = 0; i < num_cols; i++)
        {
            if (!data[i] || (skip && (*skip)[i]))
                continue;
            if (on_cpu)
                free(data[i]);
            else if (on_um)
                CHECK_CUDA_ERROR(cudaFree(data[i]));
            else
                release_mem(data[i], st_);
            data[i] = nullptr;
        }
        if (gpu_data_)
        {
    ...
    __host__ auto consolidate(bool to_cpu = true)
    {
        assert(!on_cpu);
        CHECK_LAST_CUDA_ERROR();
        ...
        for (int i = 0; i < num_cols; i++)
        {
            ...
            {
                if (on_um)
                    CHECK_CUDA_ERROR(cudaFree(to_free[i]));
                else
                    release_mem(to_free[i]);
            }
        }
    ...
    __host__ auto add_column(void *col_ptr)
    {
    ...
    __host__ void set_consolidated(bool c) { consolidated = c; }

    __host__ bool is_consolidated() { return consolidated; }

    // for debugging
    __host__ int get_selected_items_num()
    {
        if (!select_vec)
        {
    ...
    __host__ void allocate_um(int N)
    {
        assert(empty());
        for_<num_cols>([&](auto i)
                       {
    ...
    __host__ void allocate_cpu(int N)
    {
        assert(empty());
        for_<num_cols>([&](auto i)
                       {
    ...
    __host__ void allocate_select_vec()
    {
    ...
    __host__ void set_num_items(int n)
    {
        num_items = n;
        capacity = max(capacity, n);
    }

    __host__ auto &send_data_to_gpu_async(cudaStream_t st)
    {
        assert(on_cpu && !on_um);
        if (num_items == 0)
        {
...
private:
    ...
    __host__ auto &send_data_to_cpu()
    {
        assert(!on_cpu);
        if (on_um || num_items == 0)
        {
            return *this;
        }
