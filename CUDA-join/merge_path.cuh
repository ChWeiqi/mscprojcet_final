...
struct merge_range
{
    ...
    __device__ merge_range(const idx_t w, const idx_t x, const idx_t y, const idx_t z)
        : a_begin(w), a_end(x), b_begin(y), b_end(z) {}
                                     ...
                                     int *g_counter, int circular_buffer_size, bool agg_only = false)
{
    ...
    for (int i = 0; i < VT; i++)
    {
        ...
        if (index >= mr_global.total())
            break;
        if (index < mr_global.a_count())
            __pipeline_memcpy_async(&keys[index], &a[index], sizeof(key_t));
        else
            __pipeline_memcpy_async(&keys[index], &b[index - mr_global.a_count()], sizeof(key_t));
    }
    __pipeline_commit();
...
#pragma unroll
    ...
    while (true)
    {
        ...
        if (mask == 0)
            break; // no one has the write intention anymore, exit
