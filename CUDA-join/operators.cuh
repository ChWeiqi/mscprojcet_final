...
private:
    Tuple next_()
    {
        ...
        if (t.select_vec == nullptr)
            t.allocate_select_vec();
...
public:
    InnerEqJoinOperator(LeftUpstream b_up,
        ...
        : Operator("InnerEqJoinOperator"), l_upstream_(b_up), r_upstream_(p_up), vec_size_(vec_size), algo_name_(algo_name), profile_(!profile_output.empty()), profile_output_(profile_output)
    {
        cudaStreamSynchronize(0);
    }
